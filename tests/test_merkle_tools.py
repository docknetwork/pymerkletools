...
def test_ethereum_keccak_256():
    test_str = b'0x'
    expected_digest = '39bef1777deb3dfb14f64b9f81ced092c501fee72f90e93d03bb95ee89df9837'
    digest = sha3.keccak_256(string=test_str).hexdigest()
    assert digest == expected_digest


def test_add_leaf():
    mt = MerkleTools(DEFAULT_HASH_FUNCTION)
...
def test_sha384():
    ...
    assert mt.get_proof(
        0)[0]['right'] == '368c89a00446010def75ad7b179cea9a3d24f8cbb7e2755a28638d194809e7b614eb45453665032860b6c1a135fb6e8b'
    is_valid = mt.validate_proof(mt.get_proof(0), '84ae8c6367d64899aef44a951edfa4833378b9e213f916c5eb8492cc37cb951c726e334dace7dbe4bb1dc80c1efe33d0',
                                 'c363aa3b824e3f3b927034fab826eff61a9bfa2030ae9fc4598992edf9f3e42f8b497d6742946caf7a771429eb1745cf')
    assert is_valid


def test_sha512():
    mt = MerkleTools('sha512')
    mt.add_leaf([
    ...
    assert mt.get_proof(0)[
        0]['right'] == 'df1e07eccb2a2d4e1b30d11e646ba13ddc426c1aefbefcff3639405762f216fdcc40a684f3d1855e6d465f99fd9547e53fa8a485f18649fedec5448b45963976'
...
def test_sha3_384():
    ...
    assert mt.get_proof(
        0)[0]['right'] == 'ae331b6f8643ed7e404471c81be9a74f73fc84ffd5140a0ec9aa8596fa0d0a2ded5f7b780bb2fbfc4e2226ee2a04a2fa'
    is_valid = mt.validate_proof(mt.get_proof(0), 'e222605f939aa69b964a0a03d7075676bb3dbb40c3bd10b22f0adcb149434e7c1085c206f0e3371470a49817aa6d5b16',
                                 'bd54df0015fa0d4fee713fbf5c8ae232c93239c75fb9d41c7dd7a9278711764a6ee83c81766b3945ed94030254537b57')
    assert is_valid


def test_sha3_512():
    mt = MerkleTools('sha3_512')
    mt.add_leaf([
    ...
    assert mt.get_proof(0)[
        0]['right'] == '0b43a85d08c05252d0e23c96bc6b1bda11dfa787049ff452b3c86f4c6135e870c058c05131f199ef8619cfac937a736bbc936a667e4d96a5bf68e4056ce5fdce'
...
def test_blake2s():
    ...
    l1 = 'l1s'
    l2 = 'l2s'
    l11 = hash_func(l1.encode())
    l21 = hash_func(l2.encode())
    r = hash_func(l11.digest() + l21.digest())

    mt = MerkleTools(hash_type)
    mt.add_leaf([l1, l2], True)
    mt.make_tree()
    assert mt.get_merkle_root() == r.hexdigest()
    assert mt.get_proof(0)[0]['right'] == l21.hexdigest()
    assert mt.get_proof(1)[0]['left'] == l11.hexdigest()

    assert mt.validate_proof(mt.get_proof(0), l11.hexdigest(), r.hexdigest())
    assert mt.validate_proof(mt.get_proof(1), l21.hexdigest(), r.hexdigest())
...
def test_keccak_224():
    hash_type = "keccak_224"
    hash_func = getattr(sha3, hash_type)
